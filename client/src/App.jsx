...
function App() {
  ...
  // ─── Settings Handlers ───
  // Player selection removed - system Intent Chooser is always used
  ...
  const handlePlay = async (infoHash, fileIndex, fileName) => {
    ...
    if (torrent) {
      ...
      nextFile = videoFiles[currentIdx + 1]
      const playData = {
        infoHash,
        fileIndex,
        fileName,
    ...
    console.log(`[Play] URL: ${streamUrl} | Title: ${title} | Using system chooser`)

    setBuffering({ name: title, progress: 10 })
    setSelectedTorrent(null)

    try {
      // Always use empty package - let system show Intent Chooser
      const result = await TVPlayer.play({
        url: streamUrl,
        package: '',  // System will show app chooser
        title: title,
        position: 0
      })
  ...
  const handlePlayAll = async (torrent, startIndex = 0) => {
    ...
    console.log(`[PlayAll] ${urls.length} files | Using system chooser`)
